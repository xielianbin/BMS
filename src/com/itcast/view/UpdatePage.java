...
public class UpdatePage extends Page {
	...
	// 初始化图书操作子菜单的名称
	private String[] menuItemString = { "查  询", "添  加", "删  除" };
	...
	public void init() {
		...
		// 初始化添加按钮
		updateButton.setBounds(400, 460, 80, 30);
		this.add(updateButton);
	}

	public UpdatePage(Book book) {

		this.setLayout(null);
		this.setTitle("图书管理系统-图书修改界面");
		init();
		this.setResizable(false);
		this.setBounds(width, height, sizeX, sizeY);
		this.setDefaultCloseOperation(EXIT_ON_CLOSE);
		this.bookID = book.getId();
		insertPageField[0].setText(book.getName());
		insertPageField[1].setText(book.getAuthor());
		insertPageField[2].setText(book.getTime());
		insertPageField[3].setText(book.getPublish());
		insertPageField[4].setText(String.valueOf(book.getPrice()));
		insertPageField[5].setText(book.getIsbn());
		contentArea.setText(book.getContent());
		authorsArea.setText(book.getAuthors());
	}
}
